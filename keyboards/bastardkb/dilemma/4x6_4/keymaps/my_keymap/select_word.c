...
static void select_word_in_dir(int8_t dir) {
    ...
    if (selection_dir == 0) { // Initial selection.
        send_keyboard_report();
        send_string_with_delay_P((dir < 0) ? PSTR(SS_TAP(X_LEFT) SS_TAP(X_RGHT)) : PSTR(SS_TAP(X_RGHT) SS_TAP(X_LEFT)), TAP_CODE_DELAY);
    }
...
static void select_line(void) {
    ...
    if (selection_dir != 2) {
        send_keyboard_report();
        send_string_with_delay_P(IS_MAC ? PSTR(SS_LGUI(SS_TAP(X_LEFT) SS_LSFT(SS_TAP(X_RGHT)))) : PSTR(SS_TAP(X_HOME) SS_LSFT(SS_TAP(X_END))), TAP_CODE_DELAY);
    } else {
        register_mods(MOD_BIT_LSHIFT);
