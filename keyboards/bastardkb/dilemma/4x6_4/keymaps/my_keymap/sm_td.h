...
#define EMPTY_STATE {.pressed_keyposition = MAKE_KEYPOS(0, 0), .pressed_keycode = 0, .desired_keycode = 0, .saved_mods = 0, .sequence_len = 0, .pressed_time = 0, .released_time = 0, .timeout = INVALID_DEFERRED_TOKEN, .stage = SMTD_STAGE_NONE, .resolution = SMTD_RESOLUTION_UNCERTAIN, .next_action = SMTD_ACTION_TOUCH, .need_next_action = false, .idx = 0}
...
static smtd_state  smtd_states_pool[SMTD_POOL_SIZE]   = {EMPTY_STATE, EMPTY_STATE, EMPTY_STATE, EMPTY_STATE, EMPTY_STATE, EMPTY_STATE, EMPTY_STATE, EMPTY_STATE, EMPTY_STATE, EMPTY_STATE};
...
char *smtd_state_to_str(smtd_state *state) {
    ...
    snprintf(buffer_state, sizeof(buffer_state), "S[%d](@%d.%d#%s->%s){%s/%s,m=%x}", state->idx, state->pressed_keyposition.row, state->pressed_keyposition.col, smtd_keycode_to_str(state->pressed_keycode), smtd_keycode_to_str(state->desired_keycode), smtd_stage_to_str(state->stage), smtd_resolution_to_str(state->resolution), state->saved_mods);
...
char *smtd_state_to_str2(smtd_state *state) {
    ...
    snprintf(buffer_state2, sizeof(buffer_state2), "S[%d](@%d.%d#%s->%s){%s/%s,m=%x}", state->idx, state->pressed_keyposition.row, state->pressed_keyposition.col, smtd_keycode_to_str(state->pressed_keycode), smtd_keycode_to_str(state->desired_keycode), smtd_stage_to_str(state->stage), smtd_resolution_to_str(state->resolution), state->saved_mods);
...
char *smtd_record_to_str(keyrecord_t *record) {
    ...
    snprintf(buffer_record, sizeof(buffer_record), "R(@%d#%d %s)", record->event.key.row, record->event.key.col, record->event.pressed ? "|*|" : "|O|");
...
bool smtd_process_desired(uint16_t pressed_keycode, keyrecord_t *record, uint16_t desired_keycode) {
    if (smtd_bypass) {
        SMTD_DEBUG("            %s GLOBAL BYPASS KEY %s\n", smtd_record_to_str(record), smtd_keycode_to_str_uncertain(pressed_keycode, desired_keycode == 0));
        return true;
    }

#ifdef SMTD_DEBUG_ENABLED
    SMTD_DEBUG("\n>> +%lums %s GOT KEY %s\n", timer_elapsed32(last_key_timer), smtd_record_to_str(record), smtd_keycode_to_str_uncertain(pressed_keycode, desired_keycode == 0));
...
void smtd_apply_to_stack(uint8_t starting_idx, uint16_t pressed_keycode, keyrecord_t *record, uint16_t desired_keycode) {
    ...
    for (uint8_t i = starting_idx; i < smtd_active_states_size; i++) {
        ...
        bool is_state_key = (record->event.key.row == state->pressed_keyposition.row && record->event.key.col == state->pressed_keyposition.col) && (pressed_keycode == state->pressed_keycode || pressed_keycode == state->desired_keycode);
        ...
        if (!can_process_next) {
            SMTD_DEBUG("<< %s TERM STATE with %s\n", smtd_record_to_str(record), smtd_state_to_str(state));
            break;
        }
    }
...
bool smtd_apply_event(bool is_state_key, smtd_state *state, uint16_t pressed_keycode, keyrecord_t *record, uint16_t desired_keycode) {
    SMTD_DEBUG("   -- %s apply_event with %s, is_state_key=%d\n", smtd_state_to_str(state), smtd_record_to_str(record), is_state_key);
        ...
        case SMTD_STAGE_FOLLOWING_TOUCH:
            ...
            if (!is_state_key && !record->event.pressed) {
                ...
                for (uint8_t i = state->idx + 1; i < smtd_active_states_size; i++) {
                    bool is_following_state_key = (record->event.key.row == smtd_active_states[i]->pressed_keyposition.row && record->event.key.col == smtd_active_states[i]->pressed_keyposition.col) && (pressed_keycode == smtd_active_states[i]->pressed_keycode || pressed_keycode == smtd_active_states[i]->desired_keycode);
...
void smtd_apply_stage(smtd_state *state, smtd_stage next_stage) {
    SMTD_DEBUG("      %s stage -> %s\n", smtd_state_to_str(state), smtd_stage_to_str(next_stage));
    ...
    switch (state->stage) {
        ...
        case SMTD_STAGE_TOUCH:
            ...
            state->timeout      = defer_exec(get_smtd_timeout_or_default(state, SMTD_TIMEOUT_TAP), timeout_touch, state);
            SMTD_DEBUG("      %s timeout_touch in %lums\n", smtd_state_to_str(state), get_smtd_timeout_or_default(state, SMTD_TIMEOUT_TAP));
            break;
        ...
        case SMTD_STAGE_SEQUENCE:
            ...
            state->resolution    = SMTD_RESOLUTION_UNCERTAIN;
            state->saved_mods    = get_mods();
            state->timeout       = defer_exec(get_smtd_timeout_or_default(state, SMTD_TIMEOUT_SEQUENCE), timeout_sequence, state);
            SMTD_DEBUG("      %s timeout_sequence in %lums\n", smtd_state_to_str(state), get_smtd_timeout_or_default(state, SMTD_TIMEOUT_SEQUENCE));
            break;
        ...
        case SMTD_STAGE_FOLLOWING_TOUCH:
            state->timeout = defer_exec(get_smtd_timeout_or_default(state, SMTD_TIMEOUT_FOLLOWING_TAP), timeout_following_touch, state);
            SMTD_DEBUG("      %s timeout_following_touch in %lums\n", smtd_state_to_str(state), get_smtd_timeout_or_default(state, SMTD_TIMEOUT_FOLLOWING_TAP));
            break;

        case SMTD_STAGE_RELEASE:
            state->released_time = timer_read32();
            state->timeout       = defer_exec(get_smtd_timeout_or_default(state, SMTD_TIMEOUT_RELEASE), timeout_release, state);
            SMTD_DEBUG("      %s timeout_release in %lums\n", smtd_state_to_str(state), get_smtd_timeout_or_default(state, SMTD_TIMEOUT_RELEASE));
            break;
    }
...
void smtd_handle_action(smtd_state *state, smtd_action action) {
    SMTD_DEBUG("        %s action processing with %s\n", smtd_state_to_str(state), smtd_action_to_str(action));
    ...
    SMTD_DEBUG("        %s action with %s runs deferred %s\n", smtd_state_to_str(state), smtd_action_to_str(action), smtd_state_to_str2(next_state));
...
void smtd_propagate_mods(smtd_state *state, uint8_t mods_before_action, uint8_t mods_after_action) {
    ...
    SMTD_DEBUG("            %s mods: before:%x  after:%x  ^^:%x  ++:%x  --:%x\n", smtd_state_to_str(state), mods_before_action, mods_after_action, changed_mods, enabled_mods, disabled_mods);

    for (uint8_t i = state->idx + 1; i < smtd_active_states_size; i++) {
        SMTD_DEBUG("            %s mods upd %s by +%x -%x\n", smtd_state_to_str(state), smtd_state_to_str2(smtd_active_states[i]), enabled_mods, disabled_mods);

        smtd_active_states[i]->saved_mods |= enabled_mods;
        smtd_active_states[i]->saved_mods &= ~disabled_mods;

        SMTD_DEBUG("            %s changed mods %s\n", smtd_state_to_str(state), smtd_state_to_str2(smtd_active_states[i]));
    }
}
...
void smtd_emulate_press(keypos_t *keypos, bool press) {
    SMTD_DEBUG("            EMULATE %s %s\n", press ? "PRESS" : "RELEASE", smtd_keycode_to_str(smtd_current_keycode(keypos)));
    smtd_bypass = true;
...
#define SMTD_MT5_ON_MKEY(macro_key, tap_key, mod, threshold, use_cl) SMTD_DANCE(macro_key, NOTHING, SMTD_TAP_16(use_cl, tap_key), SMTD_LIMIT(threshold, register_mods(MOD_BIT(mod)); send_keyboard_report(), SMTD_REGISTER_16(use_cl, tap_key)), SMTD_LIMIT(threshold, unregister_mods(MOD_BIT(mod)); send_keyboard_report(), SMTD_UNREGISTER_16(use_cl, tap_key)); send_keyboard_report())
...
#define SMTD_MTE5_ON_MKEY(macro_key, tap_key, mod, threshold, use_cl) SMTD_DANCE(macro_key, EXEC(register_mods(MOD_BIT(mod)); send_keyboard_report();), EXEC(unregister_mods(MOD_BIT(mod)); SMTD_TAP_16(use_cl, tap_key);), SMTD_LIMIT(threshold, NOTHING, EXEC(unregister_mods(MOD_BIT(mod)); send_keyboard_report(); SMTD_REGISTER_16(use_cl, tap_key);)), SMTD_LIMIT(threshold, EXEC(unregister_mods(MOD_BIT(mod)); send_keyboard_report();), SMTD_UNREGISTER_16(use_cl, tap_key)))
...
#define SMTD_LT5_ON_MKEY(macro_key, tap_key, layer, threshold, use_cl) SMTD_DANCE(macro_key, NOTHING, SMTD_TAP_16(use_cl, tap_key), SMTD_LIMIT(threshold, LAYER_PUSH(layer), SMTD_REGISTER_16(use_cl, tap_key)), SMTD_LIMIT(threshold, LAYER_RESTORE(), NOTHING); SMTD_UNREGISTER_16(use_cl, tap_key))
...
#define SMTD_TD5_ON_MKEY(macro_key, tap_key, hold_key, threshold, use_cl) SMTD_DANCE(macro_key, NOTHING, SMTD_TAP_16(use_cl, tap_key), SMTD_LIMIT(threshold, SMTD_TAP_16(use_cl, hold_key), SMTD_TAP_16(use_cl, tap_key)), SMTD_LIMIT(threshold, SMTD_UNREGISTER_16(use_cl, hold_key), SMTD_UNREGISTER_16(use_cl, tap_key)))
...
#define SMTD_TK4_ON_MKEY(macro_key, tap_key, threshold, use_cl) SMTD_DANCE(macro_key, SMTD_LIMIT(threshold, NOTHING, SMTD_TAP_16(use_cl, tap_key)), NOTHING, NOTHING, NOTHING)
...
#define SMTD_TTO4_ON_MKEY(macro_key, layer, threshold, use_cl) SMTD_DANCE(macro_key, SMTD_LIMIT(threshold, NOTHING, layer_move(layer)), NOTHING, NOTHING, NOTHING)
