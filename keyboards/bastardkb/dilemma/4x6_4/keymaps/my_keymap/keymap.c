...
#include "features/select_word.h" // <-- Added for the select-word feature
...
// Step 1: Create a custom keycode for your select-word macro
enum custom_keycodes {
    SELWORD = SAFE_RANGE,
    // Other custom keys can go here...
};

// Define your usual layer-keycode aliases
#define LOWER MO(LAYER_LOWER)
#define RAISE MO(LAYER_RAISE)
#define POINT MO(LAYER_POINTER)

// If you don't enable POINTING_DEVICE_ENABLE, these are no-ops
...
// Step 2: Handle the new keycode in process_record_user() by calling process_select_word()
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    // If process_select_word() returns false, it has handled the key—stop processing.
    if (!process_select_word(keycode, record, SELWORD)) {
        return false;
    }

    // If you have other custom macros or keycodes, handle them here:
    switch (keycode) {
        // Add other cases as needed...
    }

    return true; // Let QMK continue processing any remaining logic
}
